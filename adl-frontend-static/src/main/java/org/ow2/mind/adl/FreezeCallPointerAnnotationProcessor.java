 ...
 *  
 */
public class FreezeCallPointerAnnotationProcessor extends
AbstractADLLoaderAnnotationProcessor {

	protected static Logger        sscpLogger        = FractalADLLogManager
			.getLogger("sscp");

	/**
	 * Inspired from StaticDefinitionsBindingsListAnnotationProcessor
	 */
	public Definition processAnnotation(Annotation annotation, Node node,
		...
		int idx = sscpAnno.toItf.lastIndexOf(".");
		String targetDefName = sscpAnno.toItf.substring(0, idx);
		String targetItfInstanceName = sscpAnno.toItf.substring(idx + 1, sscpAnno.toItf.length());

		try {
			targetDef = loaderItf.load(targetDefName, context);
		} catch (ADLException e) {
			sscpLogger.log(Level.WARNING, targetDefName + " definition not found, @SingleStaticCallPointer(" + sscpAnno.toItf + ") skipped !");
			return null;
		}
		...
		for (Interface currItf : targetDefItfs) {
			if (currItf.getName().equals(targetItfInstanceName)) {
				foundTargetItfInstance = true;
				resolvedInterface = currItf;
				break;
			}
		}

		final InterfaceDefinition itfDef = InterfaceDefinitionDecorationHelper
						.getResolvedInterfaceDefinition((TypeInterface) resolvedInterface, idlLoaderItf, context);

		// Propagate the decoration for the .optimized_macro StringTemplate generator to write the good CALL_PTR function call.
		// Also for the .inc files to specify the according function prototype
		// here we abuse (just a little bit) the naming convention system
		if (foundTargetItfInstance) {
			definition.astSetDecoration("single-static-call-ptr-targetinstance", "__component_" + targetDefName.replace(".", "_") + "_" + targetItfInstanceName);
			definition.astSetDecoration("single-static-call-ptr-targetitfdef", itfDef);
		}
